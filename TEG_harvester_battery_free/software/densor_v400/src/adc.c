 ...
 *
 * VMEAS is connected to a voltage divider. Turning on VMEAS allows to measure VDD-BAT
 */
void VMEAS_On(){
...
/**
 * @brief: Function for turning off VMEAS
 */
void VMEAS_Off(){
...
int measure_VDD(){
	...
	int adc_voltage = (int) (((float) adc_value / 4096) * 1000);
	/* Convert ADC voltage to VDD */
	int vdd_voltage = (int) (adc_voltage * (( (float) VMEAS_R1 + (float) VMEAS_R2)/ (float) VMEAS_R2));
